...
3. Directly below the coefficient and standard-error block, use this statistics order: `Pre-treatment mean of Y`, `Number of Clusters`, observation count, then `R-squared` (or a clearly named estimator-appropriate fit statistic). For PPML, distinguish `Final Observations` (the fitted estimation sample) and `Initial Observations` (observations submitted to the PPML estimator before estimator-specific exclusions); place both rows, in that order, before any fit statistic, and define the counts in the note. For OLS and other ordinary regressions, use one row labeled `Observations`; do not add initial/final rows by default. If the design has no well-defined pre-treatment period, label the first row `Mean of Y` instead. If inference is not clustered, omit the cluster row. Do not invent an ordinary R-squared for PPML or another estimator that does not report one; use an explicitly defined pseudo R-squared only when available, otherwise omit that row.
...
`esttab`/`estout` can stage an OLS model-by-column CSV under `output/raw/` with `stats(pre_mean_y n_clusters N r2, labels("Pre-treatment mean of Y" "Number of Clusters" "Observations" "R-squared"))` when those statistics apply. If no pre-treatment period exists, replace `pre_mean_y` and its label with a correctly computed full-sample `mean_y` and `Mean of Y`. For PPML, record the per-column input count before the estimator runs and the final fitted count after its exclusions; export both as separately labeled rows. Do not relabel a single `e(N)` as both counts. Build the consolidated XLSX from staged results and Table 1 data; confirm one complete three-line table and concrete note per sheet. Use `esttab` TeX export or a genuine Word writer/converter only if those formats are requested. Changing a filename extension does not convert a file format; do not use `outreg2 ..., tex` with a `.doc` filename.
